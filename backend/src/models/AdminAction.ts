...
export interface IAdminActionModel extends Model<IAdminActionDocument> {
  ...
  getGeneralAdminActivityStats(days: number): Promise<{
    totalActions: number;
    actionsByType: Record<string, number>;
...
export class AdminActionModel {
  ...
  private static createModel(): IAdminActionModel {
    ...
    adminActionSchema.statics['getGeneralAdminActivityStats'] = async function(days: number): Promise<{
      totalActions: number;
      actionsByType: Record<string, number>;
      actionsByAdmin: Record<string, number>;
      recentActions: number;
      ...
      const result = stats[0] || { totalActions: 0, actionsByType: [], actionsByAdmin: [] };
      ...
      // Calculate actions by admin
      const actionsByAdmin: Record<string, number> = {};
      result.actionsByAdmin.forEach((item: any) => {
        actionsByAdmin[item.adminId] = (actionsByAdmin[item.adminId] || 0) + 1;
      });
