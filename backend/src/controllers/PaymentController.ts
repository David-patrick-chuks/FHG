...
export class PaymentController {
  ...
  public static async getAllPayments(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Parse and validate parameters
      const pageNum = Math.max(1, parseInt(page as string) || 1);
      const limitNum = Math.min(100, Math.max(1, parseInt(limit as string) || 10)); // Max 100 items per page for admin
      const sortOrderValue = sortOrder === 'asc' ? 'asc' : 'desc';
      ...
      const result = await PaystackService.getAllPayments(options);
      ...
      PaymentController.logger.error('Error in getAllPayments controller:', {
        message: error?.message || 'Unknown error',
        stack: error?.stack,
        name: error?.name
      });
